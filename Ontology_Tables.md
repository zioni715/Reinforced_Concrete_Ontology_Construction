...
IRI local name은 Turtle 작성 시 `ppdoc:` prefix 뒤에 붙는 local part 후보이다. 전체 고유식별자는 namespace IRI와 local name이 결합된 IRI로 확장된다. 실제 namespace IRI는 추후 다음과 같이 둘 수 있다.

```turtle
@prefix ppdoc: <https://example.org/rc-progress-payment#> .
```

RDFS의 `rdfs:domain`, `rdfs:range`는 추론용 기본 범위로만 사용한다. 현재 구축 범위는 RDF/RDFS까지이며, 필수성, cardinality, 조건부 포함 검증, 값 범위 검증은 후속 확장 범위로 둔다.

---

## 2. 핵심 클래스 용어표

| 한국어 용어 | IRI local name | 구분 | 상위 클래스 후보 | 설명 |
|---|---|---|---|---|
...
| 철근 수량산출서 | RebarQuantityCalculationSheet | Class | QuantityCalculationSheet | 위치, 부재명, 철근 규격, 철근 직경, 길이, 개수, 단위중량, 산출중량 필드를 가진 문서 |
| 콘크리트 수량산출서 | ConcreteQuantityCalculationSheet | Class | QuantityCalculationSheet | 위치, 부재명, 콘크리트 강도, 길이, 폭, 높이, 산출체적 필드를 가진 문서 |
| 거푸집 수량산출서 | FormworkQuantityCalculationSheet | Class | QuantityCalculationSheet | 위치, 부재명, 거푸집 종류, 길이, 높이, 산출면적 필드를 가진 문서 |
...
| 한국어 필드 | IRI local name | 권장 속성 유형 | 권장 range | 단위 또는 기준 |
|---|---|---|---|---|
...
| 한국어 관계 라벨 | IRI local name | 속성 유형 | 관계 방향 | 의미 |
|---|---|---|---|---|
...
| Property IRI | rdfs:domain | rdfs:range | 역관계 후보 | 비고 |
|---|---|---|---|---|
...
| Property IRI | rdfs:domain | rdfs:range | 적용 대상 | 비고 |
|---|---|---|---|---|
...
5. 필수 문서 여부, 조건부 포함 여부, 단위 일관성, 값 범위는 RDF/RDFS만으로 직접 검증하지 않고 후속 확장 범위로 둔다.
